...
const MobileHeaderContainer = styled.div<{ $isOpen: boolean; $color: "WHITE" | "BLACK" }>`
    ...
    ${(props) => props.$color === "WHITE" && "background-color: #FFFFFF20"};
    ${(props) => props.$color === "BLACK" && "background-color: #14141450"};
...
const MenuBox = styled.div<{ $isOpen: boolean }>`
    height: ${(props) => (props.$isOpen ? "calc(100vh - 58px)" : 0)};
    padding: ${(props) => (props.$isOpen ? "50px 40px" : "0 40px")};
    width: 100%;
    overflow-x: hidden;
    overflow-y: hidden;
    display: flex;
