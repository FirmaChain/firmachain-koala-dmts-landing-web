...
const Home: NextPage<IProps> = ({ articles }) => {
  ...
  const handleScroll = () => {
    ...
    if (bannerRef.current !== null && bannerRef.current.getBoundingClientRect().top <= 50)
      return setHeaderLogoColor('WHITE');
    if (articleRef.current !== null && articleRef.current.getBoundingClientRect().top <= 50)
      return setHeaderLogoColor('BLACK');
    if (technologyRef.current !== null && technologyRef.current.getBoundingClientRect().top <= 50)
      return setHeaderLogoColor('BLACK');
    if (gameIntroRef.current !== null && gameIntroRef.current.getBoundingClientRect().top <= 50)
      return setHeaderLogoColor('WHITE');
    if (size.width >= 1180) {
      if (brandIntroRef.current !== null && brandIntroRef.current.getBoundingClientRect().top <= 225)
        return setHeaderLogoColor('BLACK');
    } else {
      if (brandIntroRef.current !== null && brandIntroRef.current.getBoundingClientRect().top <= 50)
        return setHeaderLogoColor('BLACK');
    }
    if (mainVisualRef.current !== null && mainVisualRef.current.getBoundingClientRect().top <= 50)
      return setHeaderLogoColor('WHITE');
  };
